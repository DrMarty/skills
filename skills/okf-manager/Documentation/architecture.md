...
The graph shell uses three resizable desktop regions: a left navigation/glossary sidebar, a D3 graph canvas, and concept details. The left sidebar has independently collapsible sections separated by a draggable horizontal splitter: concept search and the hierarchical navigation tree above, and the generated catalog glossary below. Glossary source links select the corresponding graph concept.

Type frontmatter uses ` / ` as the canonical hierarchy separator. `okf_visualize_bundle.py` converts complete type paths into a deterministic recursive `typeTree` payload of arbitrary depth. The navigation UI renders that payload recursively: concepts assigned directly to a parent type appear in that branch, child types occupy the next deeper tree level, and parent controls aggregate every direct and descendant concept. Flat type names remain single-level branches. The tree has global, type, and concept visibility controls with live aggregate counts and starts with only its top-level Types branch expanded.

The detail panel keeps concept metadata and back/forward history above independently collapsible, vertically resizable Outgoing Links, Backlinks, and Body Preview sections. Body Preview renders a safe Markdown subset, and links in every detail section follow the same concept-selection path. Canvas controls provide zoom, fit, reset-unpin-refit, and dismissible help. The header About control displays the generator name, manifest-derived plugin version, and canonical GitHub installation/update link. Generated graph, glossary, recursive type-tree, and generator metadata remain embedded in `viz.html`; D3 is loaded from jsDelivr when the page opens.
